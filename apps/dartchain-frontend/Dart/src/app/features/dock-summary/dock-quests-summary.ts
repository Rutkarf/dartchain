...
import {
  DockQuestsPhase,
  DockQuestsStateService,
} from '../../core/services/dock-quests-state.service';
...
export class DockQuestsSummaryComponent implements OnInit, OnDestroy {
  ...
  readonly claimableCount = this.state.claimableCount;
  ...
  statusClass(phase: DockQuestsPhase): string {
    const map: Record<DockQuestsPhase, string> = {
      error: 'error',
      loading: 'busy',
      ready: 'ready',
      urgent: 'waiting',
  ...
  onOpen(event: Event): void {
    ...
    window.dispatchEvent(
      new CustomEvent('dock-open-panel', { detail: { panel: 'quests' } })
    );
  }
}
