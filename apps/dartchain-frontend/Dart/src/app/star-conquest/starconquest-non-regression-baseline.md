...
`app.ts` / `app.html` / `app.css`, `ProductConfigService` / `environment.*`, `three-webgl.util`, `three-animation.util`, `three-container.util`, `knowledge-graph/**`, `metaverse/**`, Dock/auth/wallet, viewport-compact, design tokens, package.json.
