...
class AuthServicePostgresTest {
    ...
    void registerLoginAndMePersistAcrossSessionStore() {
        authService.register(new RegisterRequest("pguser", "pguser@dartchain.dev", "password123"));

        var loginResponse = authService.login(new LoginRequest("pguser", "password123"));
    ...
    void linksWalletToAccountInPostgres() throws Exception {
        authService.register(new RegisterRequest("pgalice", "pgalice@dartchain.dev", "password123"));
        var loginResponse = authService.login(new LoginRequest("pgalice", "password123"));
